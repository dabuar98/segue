...
Parameters:
    input_file (JSON): A JSON file containing the audio features
Returns:
...
def build_index_vector(input_file):
    ...
    tmp_list.append(
        map_key(descriptors.get('tonal').get('key_key'))
    )
    tmp_list.append(
        map_key_scale(descriptors.get('tonal').get('key_scale'))
    )
    tmp_list.append(descriptors.get('tonal').get('key_strength'))
